...
int ballspeed = 20, paddlespeed = 20;
...
//Broad-Phasing
SPRITE getSweptBroadphaseRect(const SPRITE& b)
{
	SPRITE broadphasingbox;
	broadphasingbox.x = b.vx > 0 ? b.x : b.x + b.vx;
	broadphasingbox.y = b.vy > 0 ? b.y : b.y + b.vy;
	broadphasingbox.width = b.width + abs(b.vx);
	broadphasingbox.height = b.height + abs(b.vy);
	return broadphasingbox;
}
//Kiem tra va cham
...
void Game_Run(HWND hwnd)
{
	...
	Poll_Mouse();
	Poll_Keyboard();
	SPRITE broadphasebox = getSweptBroadphaseRect(ball);
	//Va cham paddle 1
	if (AABBCheck(broadphasebox, paddle1))
	{
		float normalx, normaly;
		float collisiontime = SweptAABB(ball, paddle1, normalx, normaly);
		ball.x += ball.vx * collisiontime;
		ball.y += ball.vy * collisiontime;
	...
	//Va cham paddle 2
	if (AABBCheck(broadphasebox, paddle2))
	{
		float normalx, normaly;
		float collisiontime = SweptAABB(ball, paddle2, normalx, normaly);
		ball.x += ball.vx * collisiontime;
		ball.y += ball.vy * collisiontime;
		if (collisiontime < 1.0f)
		{
